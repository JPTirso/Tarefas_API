 ...
 async testeAuth(req, res){
  ...
  async update(req,res){
    ...
    const userAntigo = await User.findById(id)
      if(!userAntigo) return res.status(404).json({message : "Usuario não encontrada"});
      if (nome && nome.trim() != "" && userAntigo.nome != nome){
        update.nome = nome
      }
      if (email && email.trim() != "" && userAntigo.email != email){
        const emailExist = await User.findOne({email: email})
        if(emailExist) return res.status(400).json({message: "Já existe um usuario com esse email"})
        update.email = email
      }
      if (password && password.trim() != "" ){
        if(!confirmPassword || confirmPassword.trim() == "") return res.status(400).json({message: "Confirme sua senha"})
        if(password != confirmPassword) return res.status(400).json({message: "A senha confirmada é diferente da senha posta"})
        const mesmaSenha = await bcrypt.compare(password, userAntigo.password)
        if(mesmaSenha) return res.status(400).json({message: "A senha não pode ser igual a anterior"})
        const passwordHash = await bcrypt.hash(password, 10)
